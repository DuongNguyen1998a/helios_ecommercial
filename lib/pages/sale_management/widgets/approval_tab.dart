...
class ApprovalTab extends StatelessWidget {
  const ApprovalTab({Key? key}) : super(key: key);
    ...
    return BlocBuilder<SaleBloc, SaleState>(builder: (context, state) {
      if (state is SaleLoading) {
        ...
        if (state.sales.isNotEmpty) {
          sales = state.sales.where((element) => element.status == 1).toList();
        }
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) => ListItem(
              ...
              action: 'approval',
              onItemClick: () {
                Navigator.pushNamed(context, AppRoutes.saleManagementDetail,
